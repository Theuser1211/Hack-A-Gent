...
> **Version:** 1.0.0  
> **Status:** Historical — **Design Proposal, NOT the implemented system**  

> ⚠️ This document describes an *aspirational* event-driven, state-machine-orchestrated
> micro-agent architecture. **That system is not what `hag run` actually executes.**
>
> The production pipeline is simpler: Devpost parsing → qualification → requirement
> extraction → `RouterEngine` task graph → code generation (LLM or template fallback)
> → typecheck/repair → browser validation → evaluation.
>
> For the real architecture, see `README.md` (Architecture / Pipeline Stages) and
> `cli/pipeline/orchestrator.ts`.
