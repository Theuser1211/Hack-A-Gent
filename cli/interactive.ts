...
export async function runInteractiveEntry(ctx: CLIContext): Promise<CLIResult> {
  ...
  // Step 8: Run dynamic interview to collect team size, hours, sponsors, budget, idea
  let interviewResult: InterviewResult | null = null;
  if (analysis) {
    try {
      const questions = generateQuestions(analysis);
      if (questions.length > 0) {
        interviewResult = await runInterview(questions, ask, analysis);
        if (interviewResult.autoGeneratedIdea) {
          info(`Auto-generated idea: ${interviewResult.autoGeneratedIdea.slice(0, 80)}...`);
        }
      }
    } catch {
      // Interview is non-critical — continue with defaults
    }
  }

  console.log();

  // Step 9: Build final context (merge interview results with manual answers)
  const context = buildContext(analysis, {
    ...
    teamSize: interviewResult?.teamSize != null ? String(interviewResult.teamSize) : null,
    hoursRemaining: interviewResult?.hoursRemaining != null ? String(interviewResult.hoursRemaining) : null,
    hasExistingRepo: hasExistingRepo ? 'yes' : (answers.hasExistingRepo || null),
  });

  // Step 10: Generate execution plan (uses actual team size, hours, stack)
  ...
  // Step 11: Auto-launch the pipeline
