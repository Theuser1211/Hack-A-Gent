...
export class WinningStrategyGenerator {
  generate(analysis: CompetitionAnalysis, interviewResult?: InterviewResult): WinningStrategy {
    ...
    const ideation = interviewResult?.ideation ?? null;
    const brandName =
    ...
    return {
      ...
      brandName,
      ideation,
      oneLiner: interviewResult?.userProjectIdea ?? interviewResult?.autoGeneratedIdea ?? ideation?.winner.oneLiner ?? `${analysis.challenge.title} — ${analysis.challenge.theme} solution targeting ${topCriteria.map(c => c.name).join(', ')}`,
      whyScoreWell: [
