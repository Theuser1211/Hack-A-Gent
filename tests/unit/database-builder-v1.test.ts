...
function createDatabaseTask(blueprint: ArchitectureBlueprint): Task {
  return createTask({
    project_id: 'proj-1',
    type: 'implementation',
    description: 'Generate database',
    creator_agent: 'agent.builder.database.v1',
    input: { blueprint },
    acceptance_criteria: [
      {
        criterion_id: 'c1',
        description: 'Database files generated',
...
describe('DatabaseBuilderAgent', () => {
  let agent: DatabaseBuilderAgent;
  let provider: MockBuilderProvider;
  let eventBus: EventBus;
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = mkdtempSync(path.join(os.tmpdir(), 'hackagent-db-'));
    provider = new MockBuilderProvider();
    eventBus = new EventBus(path.join(tmpDir, 'events'));
    await eventBus.start();

    agent = new DatabaseBuilderAgent({
      provider,
      eventBus,
      memoryWriter: new MemoryWriter(tmpDir),
      agentId: 'agent.builder.database.v1',
    });
  });
  ...
  describe('manifest', () => {
    it('has correct agent_id', () => {
      expect(agent.manifest.agent_id).toBe('agent.builder.database.v1');
    });

    it('has schema_generation capability', () => {
      expect(agent.manifest.capabilities.some((c) => c.capability_id === 'schema_generation')).toBe(true);
    });
  });
  ...
  describe('executeTask', () => {
    it('completes successfully with valid blueprint', async () => {
      ...
      const task = createDatabaseTask(blueprint);
    ...
    it('emits BUILD_STARTED event', async () => {
      ...
      const blueprint = createSampleBlueprint();
      await agent.executeTask(createDatabaseTask(blueprint));
